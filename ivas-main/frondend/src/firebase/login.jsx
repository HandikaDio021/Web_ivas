...
import logoinfoglobal from "../assets/images/company-logos/logo-infoglobal.png";
...
const Login = ({ ThemeChanger }) => {
  ...
  const Login1 = () => {
    ...
    // Validasi email infoglobal@gmail.com
    if (!emailRegex.test(email)) {
      setError("Invalid email format");
      return;
    }

    // Periksa panjang minimum kata sandi
    if (password.length < 6) {
      setError("Password must be at least 8 characters long");
      return;
    }

    // Periksa apakah pengguna telah mendaftar
    const registeredUsers = ["adminreact@gmail.com"]; // Daftar email terdaftar
    if (!registeredUsers.includes(email)) {
      setError("You have not signed up for an account");
      return;
    }
          ...
          <Col xxl={4} xl={5} lg={5} md={6} sm={8} className="col-12">
            <div className="my-5 d-flex justify-content-center">
              <Link to={`${import.meta.env.BASE_URL}dashboards/crm/`}>
                ...
                <img src={logoinfoglobal} alt="logo" className="desktop-logo" />
                <img
                  src={desktopdarklogo}
                  alt="logo"
